...
// Ported from TeeJS/git-updater build/verify-mac-build.js (same author), where every
// claim below was measured on macOS 26.6.2 arm64. Only the override variable is renamed.
...
//
// Set FWE_ALLOW_UNNOTARIZED=1 for a deliberate local build you are not shipping.
...
// A disk image is ready when it carries its own valid signature AND a stapled ticket
// (see notarize-dmg.js: a ticket alone still measured "rejected"). Both checks work
// whether or not Gatekeeper assessments are enabled on the build machine; when they
// are, spctl is asked as well, which is the check git-updater's runbook uses.
function isDmgReady(dmg) {
  const ok = (cmd, args) => {
    try {
  ...
  if (!ok('codesign', ['--verify', '--strict', dmg])) return false;
  if (!ok('xcrun', ['stapler', 'validate', dmg])) return false;
  let assessing = false;
  try {
    assessing = /enabled/.test(execFileSync('spctl', ['--status'], { encoding: 'utf8' }));
  } catch {
    assessing = false;
  }
  return !assessing || ok('spctl', ['-a', '-t', 'open', '--context', 'context:primary-signature', dmg]);
}
...
exports.default = async function verifyMacBuild(context) {
  ...
  if (process.platform !== 'darwin') {
    ...
    throw new Error(
      `macOS build produced on ${process.platform}, where its notarization cannot be verified. ` +
        'Build it on macOS, or set FWE_ALLOW_UNNOTARIZED=1 to accept an unverified artifact.'
    );
  }
  ...
  // faction-wars-editor addition: check the DISK IMAGES too. notarizeDmgs returns [] -
  // quietly - when it cannot see a Developer ID, and in CI electron-builder deletes its
  // temporary keychain (packager disposers) BEFORE this hook runs. Checking only the
  // .app would then pass while shipping an unsigned .dmg that Gatekeeper rejects.
  const badDmgs = require('./notarize-dmg')
    .dmgsIn(context && context.artifactPaths, outDir)
    .filter((d) => !isDmgReady(d))
  if (!bad.length && !badDmgs.length) {
    console.log(`  • notarization verified against ${apps.length} app(s) and the disk image(s), not the build log`);
    return;
  }
  bad.push(...badDmgs.map((d) => `${d}  (disk image: not signed, or no stapled ticket)`));
  ...
  const message =
    ...
    '  Set FWE_ALLOW_UNNOTARIZED=1 for a local build you are not shipping.';

  if (process.env.FWE_ALLOW_UNNOTARIZED === '1') {
...
module.exports.isDmgReady = isDmgReady;
